...
| **Request the intermediate transcode as MPEG-TS** | Turn on for ffmpeg-over-ip v5+. Its file tunnel lets the plugin catch an unfinished fragment header in Jellyfin's MP4 transcode, which cuts the download short; MPEG-TS is not affected. Off by default. Details in [docs/remote-encoder.md](docs/remote-encoder.md). |
...
- **Remote encoder (ffmpeg-over-ip v5): the download fails with "The transcode stream ended
  early"** → turn on **Request the intermediate transcode as MPEG-TS** under *Remote encoder*.
  Why this happens, and what was tested, is in [docs/remote-encoder.md](docs/remote-encoder.md).
